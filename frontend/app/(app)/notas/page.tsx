...
import { Plus, FileText, Eye, Pencil, Columns2, Heading1, ListChecks, Code, Image as ImageIcon, Star, Archive, Copy, Trash2 } from "lucide-react";
...
export default function NotasPage() {
  ...
  async function removeNote(note: any, e: React.MouseEvent) {
    e.stopPropagation();
      ...
      <div className="w-56 shrink-0 space-y-1 overflow-y-auto border-r border-[rgb(var(--border))] pr-3">
        ...
        {notes.map((n) => (
          ...
          >
            ...
            <button onClick={(e) => removeNote(n, e)} className="opacity-0 group-hover:opacity-100 hover:text-red-500"><Trash2 size={11} /></button>
          </div>
        ))}
      </div>
